...
class FileIter(DataIter):
    ...
    def __init__(self, root_dir, flist_name,
        ...
        self.data, self.label, self.x, self.y, self.h, self.w = self._read()
        self.cursor = -1
...
#        _, data_img_name, label_img_name = self.f.readline().strip('\n').split("\t")
    ...
    #	line = self.f.readline().split("\n")[0]
    #	data_img_name = "JPEGImages/" + line + '.jpg'
    #	label_img_name = "SegmentationClass/" + line + '.png'
        data_img_name = line[0]
        label_img_name = line[1].replace('jpg','npz')
        label_img_name = line[1].replace('png', 'npz')
    ...
    def _read_npz(self, img_name, npz_name):
        import time
        ...
        try:
            ...
            # x_center = h.resize((720, 720))
            # y_center = h.resize((720, 720))
            # h = h.resize((720, 720))
            # w = w.resize((720,720))
            img = img.resize((720,720))
        except IOError:
            print img_name + " cannot be resized"
